  ...
  for (int z=0;z<=Z;++z) // All ionization stages
    for (int i=1;i<nl[z];++i) // upper level
      for (int ii=0;ii<i;++ii){ // lower level
	      	  ...
	      	  for (int x3i=x3l;x3i<=x3h;++x3i){
              	...
              	if (nm[tr][0] == 1 + nm[tr][1] == 0 + nm[tr][2] <= 1){
              	  for (int l=1;l<=nlambda;++l){
              	  	...
              	  	op_vector[l][x1i][x2i][x3i][1][1] += op_loc * H;
              	  	em_vector[l][x1i][x2i][x3i][1] += em_loc * H;
		          ...
		          for (int l=1;l<=nlambda;++l){
                    ...
                    for (int p=1;p<=5;++p){
		              ...
		              op_vector_pert[l][p][x3i][x1i][x2i][x3i][2][3] += 0.5*(F_r_der[p]-F_b_der[p])*st * op_loc;
		              // And to emissivity:
		              em_vector[l][x1i][x2i][x3i][1] += 0.5 * (H_p_der[p] * st*st + 0.5 * (H_r_der[p] + H_b_der[p]) * (1.0+ct*ct)) * em_loc;              
	                  em_vector[l][x1i][x2i][x3i][2] += 0.5 * (H_p_der[p]-0.5*(H_r_der[p]+H_b_der[p])) * st*st*cp * em_loc;
	                  em_vector[l][x1i][x2i][x3i][3] += 0.5 * (H_p_der[p]-0.5*(H_r_der[p]+H_b_der[p])) * st*st*sp * em_loc;
	                  em_vector[l][x1i][x2i][x3i][4] += 0.5 * (H_r_der[p] - H_b_der[p]) * ct * em_loc;
                    }
          ...
          // But now, go on another loop to account for the derivatives of level populations, this loop now goes over everything
          for (int x3k=x3l;x3k<=x3h;++x3k)
      	}
  }
  return 0;
