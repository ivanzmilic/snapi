...
fp_t ***atmosphere::emissivity_active_only(fp_t ***T_in,fp_t ***Ne_in,fp_t ***Vlos,fp_t ***Vt_in,fp_t ****B,fp_t theta,fp_t phi,fp_t lambda)
{
...
fp_t ***atmosphere::emissivity_lte(fp_t ***T_in,fp_t ***Ne_in,fp_t ***Vlos,fp_t ***Vt_in,
                            fp_t ****B,fp_t theta,fp_t phi,fp_t lambda)
{
  fp_t *** em = opacity_lte(T_in, Ne_in, Vlos, Vt_in, B, theta,phi,lambda);
  for (int x1i=x1l;x1i<=x1h;++x1i)
    for (int x2i=x2l;x2i<=x2h;++x2i)
      for (int x3i=x3l;x3i<=x3h;++x3i)
...
void atmosphere::print_line_parameters(const char *filename, const char *element, int ion, int lower, int upper){
  ...
  for (int x3i=x3l; x3i<=x3h; ++x3i){
    ...
    // Line parameters
    for(int a=0;a<natm;++a)
      if (!strcmp(atml[a]->get_frm(),element)){
        // Get the damping from the dedicated function:
        fprintf(ftest, "%e ", atml[a]->get_total_line_damping(x1l,x2l,x3i,ion,lower,upper)); // Damping of the Ca I 4227 line
        // Need to fetch the populations of the lower and upper level of the 4227 line, which are 0 and 2 in the current ordering
        fprintf(ftest, "%e %e ", atml[a]->get_pop(x1l,x2l,x3i,ion,lower), atml[a]->get_pop(x1l,x2l,x3i,ion,upper)); // Ground and upper level of Ca I 4227
        // Need to fetch the collisional rates between the two levels:
        fprintf(ftest, "%e ", atml[a]->get_C_ij(x1l,x2l,x3i,ion,upper,lower));
        // What is Qel? Could be damping due to elastic collisions? # TODO
        fprintf(ftest, "%e ", atml[a]->get_damp_col(x1l,x2l,x3i,upper,lower,upper));
      }
