...
int anacrunchrun8(byte *x,byte *array,int slice,int nx,int ny,int limit,int t_endian)
 ...
 /* we have to put these in a form readable by the Vax (these may be used
	 by fcwrite) */
  if(t_endian){ // big endian
   swap(&(ch->tsize),1); swap(&(ch->bsize),1); swap(&(ch->nblocks),1); 
  }
 free(dif);
...
//  x+=sizeof(struct compresshead);
  ...
  for(int iy=0;iy<ny;++iy){               // start of iy (outer) loop
    y.w=array[in];
    ...
    for(in=iynx+1;in<iynx+nx;++in){        // start of ix (inner) loop
      ...
      if(j==0){                            // now load nb bytes into x, low order byte of y.i is first in stream
        y.i=(y.i&mask);
        x[i]=(byte)y.i;
        if(slice>8) x[i+1]=(byte)(y.i>>8); // since we started at bit 0, spillover to the next byte is determined as follows (and is unlikely since slice gt 8 is unusual
      }else{
        y.i=(y.i&mask)<<j;
        x[i]=x[i]|(byte)y.i;
