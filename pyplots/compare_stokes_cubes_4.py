...
cube2 = temp["data"]
cube3 = pyana.fzread(cube3_in)["data"]
cube4 = pyana.fzread(cube4_in)["data"]
...
del cube2
cube2=1.0
to_plot_3 = np.zeros([NX,NY,4,len(wls)])
to_plot_3[:,:,0,:] = cube3[:,:,0,wls]
to_plot_3[:,:,3,:] = cube3[:,:,3,wls+shift]
to_plot_4 = np.zeros([NX,NY,4,len(wls)])
to_plot_4[:,:,0,:] = cube4[:,:,0,wls]
to_plot_4[:,:,3,:] = cube4[:,:,3,wls+shift]
...
for j in range (1,N_y_panels):
	...
	if (j!=N_y_panels):
		ax.set_xticklabels([])
	if (j==1):
		ax.set_title('Fit using set 0')
	ax.set_yticklabels([])
	image_no+=1

	#Fitted intensity:
	to_plot = np.copy(to_plot_3[:,:,0,j-1])
	to_plot/=m
	ax=axes.flat[image_no]
	im = ax.imshow(to_plot,origin='lower',vmin = irange[0],vmax= irange[1],cmap='hot',extent=[0,x[-1],0,y[-1]])
	if (j==N_y_panels):
	...
	if (j!=N_y_panels):
		ax.set_xticklabels([])
	if (j==1):
		ax.set_title('Fit using set 1')
	ax.set_yticklabels([])
	image_no+=1

	#Fitted intensity:
	to_plot = np.copy(to_plot_4[:,:,0,j-1])
	to_plot/=m
	ax=axes.flat[image_no]
	im = ax.imshow(to_plot,origin='lower',vmin = irange[0],vmax= irange[1],cmap='hot',extent=[0,x[-1],0,y[-1]])
	if (j==N_y_panels):
...
for j in range (N_y_panels,N_y_panels+1):

	to_plot = np.copy(to_plot_1[:,:,0,j-2])
	m = np.mean(to_plot)
	to_plot/=m
	s = np.std(to_plot)

	#Observed V/I_qs
	to_plot = to_plot_1[:,:,3,j-2]/m*100.0
	
	ax = axes.flat[image_no]
	im = ax.imshow(to_plot,origin='lower',vmin = vrange[0],vmax=vrange[1],cmap='coolwarm',extent=[0,x[-1],0,y[-1]])
	if (j==N_y_panels):
		ax.set_xlabel('$x\,[\mathrm{Mm}]$')
	if (j!=N_y_panels):
		ax.set_xticklabels([])
	...
	to_plot = to_plot_2[:,:,3,j-2]/m*100.0
	...
	to_plot = to_plot_3[:,:,3,j-2]/m*100.0
	...
	to_plot = to_plot_4[:,:,3,j-2]/m*100.0
